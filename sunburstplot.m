    ...
    switch i
        ...
        case 3
            if ~isempty(varargin{3})                
                valflag = varargin{3};
                if ~ismember(valflag,[0 1 2 3])
                    error('Label value flag should be either 0, 1, 2 or 3');
                end
            end
        case 4
...
for i = 1:branches % sector plotting is done branch-wise
    ...
    if (fractang(2)-fractang(1))~=0 % plot if sector exists
        ...
        switch valflag
            ...
            case 3
                labelf=[seclabels{1}(i) num2str((nansum(table2array(subdata(:,end))) / nansum(table2array(datatree(:,end))))*100)+"%"];
        end
        ...
        for j=1:leaves1
            ...
            if (fractangl1(2)-fractangl1(1))~=0 % plot if sector exists
                ...
                switch valflag
                    ...
                    case 3
                        labelf=[seclabels{2}(j) num2str((nansum(table2array(subdata1(:,end))) / nansum(table2array(datatree(:,end))))*100)+"%"];
                end
                ...
                for k=1:leaves2
                    ...
                    if (fractangl2(2)-fractangl2(1))~=0 % plot if sector exists
                        ...
                        switch valflag
                            ...
                            case 3
                                labelf=[seclabels{3}(k) num2str((nansum(table2array(subdata2(:,end))) / nansum(table2array(datatree(:,end))))*100)+"%"];
                        end
                        ...
                        for m=1:leaves3
                            ...
                            if (fractangl3(2)-fractangl3(1))~=0 % plot if sector exists
                                ...
                                switch valflag
                                    ...
                                    case 3
                                        labelf=[seclabels{4}(m) num2str((nansum(table2array(subdata3(:,end))) / nansum(table2array(datatree(:,end))))*100)+"%"];
                                end
